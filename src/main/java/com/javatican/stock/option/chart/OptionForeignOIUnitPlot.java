...
@Component("ofoiuPlot")
public class OptionForeignOIUnitPlot {
	...
	public OptionForeignOIUnitPlot() {
	}
	...
	private XYPlot createPlot1() {
		...
		TableXYDataset dataset3 = createShortPositionOIDataset();
		// net OI axis
		NumberAxis axis1 = new NumberAxis("差额(口数)");
	...
	private TableXYDataset createLongPositionOIDataset() {
		...
		odList.stream().forEach(od -> {
			dataset.add(new Day(od.getTradingDate()), od.getCallForeignOpenLong(), "买權买方");
			dataset.add(new Day(od.getTradingDate()), od.getPutForeignOpenShort(), "卖權卖方");
		});
		return dataset;
	}
	...
	private TableXYDataset createShortPositionOIDataset() {
		...
		// add data
		odList.stream().forEach(od -> {
			dataset.add(new Day(od.getTradingDate()), -1 * od.getPutForeignOpenLong(), "卖權买方");
			dataset.add(new Day(od.getTradingDate()), -1 * od.getCallForeignOpenShort(), "买權卖方");
		});
		return dataset;
	}
	...
	private TableXYDataset createNetOIDataset() {
		...
		// add data
		odList.stream().forEach(od -> {
			dataset.add(new Day(od.getTradingDate()), od.getCallForeignOpenNet(), "买權差额(买-卖)");
			dataset.add(new Day(od.getTradingDate()), -1 * od.getPutForeignOpenNet(), "卖權差额(卖-买)");
			dataset.add(new Day(od.getTradingDate()), od.getCallForeignOpenNet() - od.getPutForeignOpenNet(), "买卖權净差额");
		});
		return dataset;
	}
