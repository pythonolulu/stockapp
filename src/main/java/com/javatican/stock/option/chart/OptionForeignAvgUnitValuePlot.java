...
@Component("ofauvPlot")
public class OptionForeignAvgUnitValuePlot extends OptionParentPlot{

	public OptionForeignAvgUnitValuePlot() {
	}
	...
	private TableXYDataset createLongPositionOIDataset() {
		...
		odList.stream().forEach(od -> {
			dataset.add(new Day(od.getTradingDate()), od.getCallForeignOpenValueLong()*1000 / od.getCallForeignOpenLong(),
					"买權买方");
			dataset.add(new Day(od.getTradingDate()), od.getPutForeignOpenValueShort()*1000 / od.getPutForeignOpenLong(),
					"卖權卖方");
		});
		return dataset;
	}
	...
	private TableXYDataset createShortPositionOIDataset() {
		...
		// add data
		odList.stream().forEach(od -> {
			dataset.add(new Day(od.getTradingDate()), -1 * od.getPutForeignOpenValueLong()*1000 / od.getPutForeignOpenLong(),
					"卖權买方");
			dataset.add(new Day(od.getTradingDate()),
					-1 * od.getCallForeignOpenValueShort()*1000 / od.getCallForeignOpenShort(), "买權卖方");
		});
		return dataset;
	}
}
