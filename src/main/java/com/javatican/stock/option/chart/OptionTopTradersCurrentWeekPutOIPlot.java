...
@Component("ottcwpoiPlot")
public class OptionTopTradersCurrentWeekPutOIPlot extends OptionParentPlot {

	public OptionTopTradersCurrentWeekPutOIPlot() {
	}
	...
	private TimeSeriesCollection createOIDataset() {
		...
		odList.stream().forEach(fd -> {
			if (fd.getPutBuyOiTop10Week() != null) {
				series1.add(new Day(fd.getTradingDate()), fd.getPutBuyOiTop10Week());
				series2.add(new Day(fd.getTradingDate()), fd.getPutBuyOiTop5Week());
				series3.add(new Day(fd.getTradingDate()), fd.getPutBuyOiTop10Week() - fd.getPutBuyOiTop5Week());
				series4.add(new Day(fd.getTradingDate()), -1 * fd.getPutSellOiTop10Week());
				series5.add(new Day(fd.getTradingDate()), -1 * fd.getPutSellOiTop5Week());
				series6.add(new Day(fd.getTradingDate()),
						-1 * (fd.getPutSellOiTop10Week() - fd.getPutSellOiTop5Week()));
			}
		});
		// add data
