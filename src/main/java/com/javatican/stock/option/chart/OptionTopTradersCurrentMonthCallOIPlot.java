...
@Component("ottcmcoiPlot")
public class OptionTopTradersCurrentMonthCallOIPlot extends OptionParentPlot {
 
	public OptionTopTradersCurrentMonthCallOIPlot() {
	}
	...
	private TimeSeriesCollection createOIDataset() {
		...
		odList.stream().forEach(fd -> {
			series1.add(new Day(fd.getTradingDate()), fd.getCallBuyOiTop10());
			series2.add(new Day(fd.getTradingDate()), fd.getCallBuyOiTop5());
			series3.add(new Day(fd.getTradingDate()), fd.getCallBuyOiTop10() - fd.getCallBuyOiTop5());
			series4.add(new Day(fd.getTradingDate()), -1 * fd.getCallSellOiTop10());
			series5.add(new Day(fd.getTradingDate()), -1 * fd.getCallSellOiTop5());
			series6.add(new Day(fd.getTradingDate()), -1 * (fd.getCallSellOiTop10() - fd.getCallSellOiTop5()));
		});
		// add data
