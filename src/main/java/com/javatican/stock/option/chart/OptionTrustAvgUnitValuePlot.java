...
@Component("otauvPlot")
public class OptionTrustAvgUnitValuePlot extends OptionParentPlot{
 
	public OptionTrustAvgUnitValuePlot() {
	}
	...
	private TableXYDataset createLongPositionOIDataset() {
		...
		odList.stream().forEach(od -> {
			dataset.add(new Day(od.getTradingDate()), od.getCallTrustOpenLong() == 0 ? 0
					: od.getCallTrustOpenValueLong() * 1000 / od.getCallTrustOpenLong(), "买權买方");
			dataset.add(new Day(od.getTradingDate()), od.getPutTrustOpenLong() == 0 ? 0
					: od.getPutTrustOpenValueShort() * 1000 / od.getPutTrustOpenLong(), "卖權卖方");
		});
		return dataset;
	}
	...
	private TableXYDataset createShortPositionOIDataset() {
		...
		odList.stream().forEach(od -> {
			dataset.add(new Day(od.getTradingDate()), od.getPutTrustOpenLong() == 0 ? 0
					: -1 * od.getPutTrustOpenValueLong() * 1000 / od.getPutTrustOpenLong(), "卖權买方");
			dataset.add(new Day(od.getTradingDate()), od.getCallTrustOpenShort() == 0 ? 0
					: -1 * od.getCallTrustOpenValueShort() * 1000 / od.getCallTrustOpenShort(), "买權卖方");
		});
		return dataset;
	}
}
