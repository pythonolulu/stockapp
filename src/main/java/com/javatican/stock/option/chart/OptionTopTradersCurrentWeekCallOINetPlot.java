...
@Component("ottcwcoinPlot")
public class OptionTopTradersCurrentWeekCallOINetPlot extends OptionParentPlot {

	public OptionTopTradersCurrentWeekCallOINetPlot() {
	}
		...
		odList.stream().forEach(fd -> {
			if (fd.getCallBuyOiTop10Week() != null) {
				...
				series3.add(new Day(fd.getTradingDate()), fd.getCallBuyOiTop10Week() - fd.getCallBuyOiTop5Week()
						- fd.getCallSellOiTop10Week() + fd.getCallSellOiTop5Week());
			}
		});
		// add data
