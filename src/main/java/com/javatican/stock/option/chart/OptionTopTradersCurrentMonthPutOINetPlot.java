...
@Component("ottcmpoinPlot")
public class OptionTopTradersCurrentMonthPutOINetPlot extends OptionParentPlot{
 
	public OptionTopTradersCurrentMonthPutOINetPlot() {
	}
	...
	private TimeSeriesCollection createOIDataset() {
		...
		odList.stream().forEach(fd -> {
			series1.add(new Day(fd.getTradingDate()), fd.getPutBuyOiTop10() - fd.getPutSellOiTop10());
			series2.add(new Day(fd.getTradingDate()), fd.getPutBuyOiTop5() - fd.getPutSellOiTop5());
