...
@Component("ottcwpoinPlot")
public class OptionTopTradersCurrentWeekPutOINetPlot extends OptionParentPlot {

	public OptionTopTradersCurrentWeekPutOINetPlot() {
	}
	...
	private TimeSeriesCollection createOIDataset() {
		...
		odList.stream().forEach(fd -> {
			if (fd.getPutBuyOiTop10Week() != null) {
				series1.add(new Day(fd.getTradingDate()), fd.getPutBuyOiTop10Week() - fd.getPutSellOiTop10Week());
				series2.add(new Day(fd.getTradingDate()), fd.getPutBuyOiTop5Week() - fd.getPutSellOiTop5Week());
