...
@Repository("priceBreakUpSelectStrategy3aDAO")
public class PriceBreakUpSelectStrategy3aDAO {
	...
	public PriceBreakUpSelectStrategy3aDAO() {
	}
	...
	public void saveResult(String dateString, Map<String, Double> resultMap) throws StockException {
		Resource resource = resourceLoader.getResource(String.format(RESULT_RESOURCE_FILE_PATH, dateString));
	...
	public Map<String, Double> loadResult(String dateString) throws StockException {
		Resource resource = resourceLoader.getResource(String.format(RESULT_RESOURCE_FILE_PATH, dateString));
		try (InputStream st = resource.getInputStream();) {
			Map<String, Double> resultMap = objectMapper.readValue(st,
	 ...
	 */
	public void save(String stockSymbol, TreeMap<Date, Double> statsMap) throws StockException {
	...
	public TreeMap<Date, Double> load(String stockSymbol) throws StockException {
		Resource resource = resourceLoader.getResource(String.format(ORIGINAL_RESOURCE_FILE_PATH, stockSymbol));
		try (InputStream st = resource.getInputStream();) {
			TreeMap<Date, Double> statsMap = objectMapper.readValue(st, new TypeReference<TreeMap<Date, Double>>() {
			});
			return statsMap;
		} catch (Exception ex) {
			logger.warn("Errror while trying to save price break up strategy '3' data for stock:" + stockSymbol);
	...
	public TreeMap<Date, Double> loadBetweenDate(String stockSymbol, Date start, Date end) throws StockException {
		TreeMap<Date, Double> statsMap = this.load(stockSymbol);
